...
public:
    ...
    */
    ...
    void distort(const Eigen::Vector2d &p_norm_undist,
                 Eigen::Vector2d &p_norm_dist) const;

    bool pixel_distortion(Eigen::Matrix3d &cameramatrix_ori,
                          Eigen::Matrix3d &cameramatrix_ideal,
                          const Eigen::Vector2d &pixel_undist,
                          Eigen::Vector2d &pixel_dist) const;

    virtual bool pixel_undistortion(Eigen::Matrix3d &cameramatrix_dist,
                                    Eigen::Matrix3d &cameramatrix_ideal,
                                    const Eigen::Vector2d &pixel_dist,
                                    Eigen::Vector2d &pixel_undist) const;

    bool undistort(const Eigen::Vector2d &p_norm_dist,
                   Eigen::Vector2d &p_norm_undist) const;

    void icvGetRectangles1(Eigen::Matrix3d &cameramatrix_ori,
                           CvSize imgSize,
                           cv::Rect_<float>& inner,
                           cv::Rect_<float>& outer) const;

    void icvGetRectangles2(Eigen::Matrix3d &cameramatrix_ori,
                           CvSize imgSize,
                           cv::Rect_<float>& inner,
                           cv::Rect_<float>& outer) const;
